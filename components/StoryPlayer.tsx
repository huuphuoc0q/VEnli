  ...
  return (
    <div className="max-w-5xl mx-auto mt-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between mb-6 px-2">
        ...
        <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-bold bg-indigo-50 dark:bg-indigo-500/10 px-4 py-2 rounded-full transition-colors">
          <BookOpen size={18} /> Story Mode
        </div>
      </div>

      <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 md:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 dark:border-slate-800 transition-colors">
        <h1 className="text-3xl md:text-4xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight mb-8 text-center">
          {story.title}
        </h1>
        ...
        <div className="flex justify-end mb-8">
          <button 
            onClick={() => setShowTranslate(!showTranslate)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-sm transition-all ${
              showTranslate ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
            }`}
          >
            <Languages size={18} /> {showTranslate ? "Chỉ hiện Tiếng Anh" : "Đọc Song Ngữ"}
          </button>
        ...
        <div className={`grid grid-cols-1 ${showTranslate ? 'lg:grid-cols-2 gap-10' : 'gap-0'}`}>
          ...
          <div className={`prose prose-lg max-w-none text-slate-700 dark:text-slate-300 leading-relaxed font-medium text-lg md:text-xl prose-mark:bg-indigo-100 dark:prose-mark:bg-indigo-500/20 prose-mark:text-indigo-800 dark:prose-mark:text-indigo-300 prose-mark:px-1.5 prose-mark:rounded-md transition-colors ${showTranslate ? 'lg:border-r lg:border-dashed lg:border-slate-200 dark:lg:border-slate-700 lg:pr-10' : ''}`}>
            {showTranslate && <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">Original (English)</h3>}
            <div dangerouslySetInnerHTML={{ __html: story.content_EN }} />
          </div>
          ...
          {showTranslate && (
            <div className="animate-in fade-in slide-in-from-right-8 duration-500">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">Bản dịch (Vietnamese)</h3>
              <div 
                className="prose prose-lg max-w-none text-slate-600 dark:text-slate-400 leading-relaxed text-lg md:text-xl italic prose-mark:bg-amber-100 dark:prose-mark:bg-amber-500/10 prose-mark:text-amber-800 dark:prose-mark:text-amber-500 prose-mark:px-1.5 prose-mark:rounded-md transition-colors"
                dangerouslySetInnerHTML={{ __html: story.content_VN }}
              />
            </div>
          )}
        ...
        <div className="mt-12 bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 transition-colors">
          <h3 className="text-sm font-bold text-slate-500 dark:text-slate-400 flex items-center gap-2 mb-4">
            <Sparkles size={16} className="text-amber-500" /> Target Vocabulary
          </h3>
          <div className="flex flex-wrap gap-2">
            {(story.vocabulary_used || []).map((vocab, idx) => (
              <span key={idx} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 px-3 py-1.5 rounded-lg font-bold shadow-sm transition-colors">
                {vocab}
              </span>
            ))}
          </div>
