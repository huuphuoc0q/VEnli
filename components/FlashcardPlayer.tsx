...
export const FlashcardPlayer: React.FC<FlashcardPlayerProps> = ({ cards, onExit }) => {
  ...
  if (!currentCard || !currentCard.word) {
    return (
      <div className="max-w-2xl mx-auto mt-20 text-center bg-white dark:bg-slate-900 p-10 rounded-2xl shadow-sm border border-red-100 dark:border-red-900/50">
        <h2 className="text-2xl font-bold text-red-500 mb-4">⚠️ Dữ liệu AI trả về bị lỗi!</h2>
        <p className="text-slate-600 dark:text-slate-400 mb-8">JSON bạn dán vào không có trường <strong>"word"</strong> hoặc sai cấu trúc.</p>
        <button onClick={onExit} className="bg-slate-800 dark:bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-slate-900 dark:hover:bg-indigo-700 transition-colors">
          Quay lại kiểm tra
        </button>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto mt-4">
      ...
      <div 
        className="bg-white dark:bg-slate-900 min-h-[400px] rounded-[2rem] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 dark:border-slate-800 flex flex-col justify-center relative transition-all duration-300 ease-in-out cursor-pointer"
        onClick={(e) => {
          const target = e.target as HTMLElement;
          if (target.tagName !== 'INPUT' && target.tagName !== 'BUTTON') {
            setIsFlipped(!isFlipped);
      ...
      >
        {!isFlipped ? (
          <div className="text-center animate-in fade-in zoom-in-95 duration-200">
            <h1 className="text-6xl font-extrabold text-[#1f2937] dark:text-white tracking-tight mb-6 transition-colors">
              {currentCard.word}
            </h1>
            <div className="flex items-center justify-center gap-3">
              {currentCard.pronunciation && (
                <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-lg font-mono text-lg transition-colors">
                  {currentCard.pronunciation}
                </span>
              )}
              <span className="px-3 py-1 bg-[#6366f1] text-white rounded-lg font-bold uppercase tracking-wider text-sm">
          ...
          <div className="w-full animate-in fade-in zoom-in-95 duration-200">
            <h2 className="text-4xl font-bold text-[#6366f1] dark:text-indigo-400 mb-2 transition-colors">{currentCard.meaningVN}</h2>
            <p className="text-slate-500 dark:text-slate-400 text-lg mb-6 transition-colors">{currentCard.definitionEN}</p>
            
            <div className="border-t border-dashed border-slate-200 dark:border-slate-700 w-full mb-6"></div>

            {currentCard.exampleEN && (
              <div className="border-l-4 border-rose-500 bg-slate-50 dark:bg-slate-800/50 p-5 rounded-r-xl mb-6 transition-colors">
                <p className="text-slate-800 dark:text-slate-200 text-lg italic mb-2">"{currentCard.exampleEN}"</p>
                <p className="text-slate-500 dark:text-slate-400">{currentCard.exampleVN}</p>
              </div>
            )}

            {currentCard.usageNote && (
              <div className="bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 p-4 rounded-xl flex gap-3 mb-6 items-start transition-colors">
                <span className="text-xl">💡</span>
                <p className="font-medium text-sm leading-relaxed">{currentCard.usageNote}</p>
              </div>
            )}
            ...
            <div className="mt-auto">
              <label className="flex items-center gap-2 font-bold text-slate-700 dark:text-slate-300 mb-3 transition-colors">
                ✍️ Thử thách: Gõ lại từ tiếng Anh
              </label>
              <div className="flex gap-3">
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && checkChallenge()}
                  className={`flex-1 p-3 border rounded-xl outline-none focus:ring-2 font-medium text-lg transition-colors bg-transparent dark:text-white
                    ${isChallengeSuccess === true ? 'border-green-500 focus:ring-green-200 dark:focus:ring-green-900/50' : ''}
                    ${isChallengeSuccess === false ? 'border-red-500 focus:ring-red-200 dark:focus:ring-red-900/50' : ''}
                    ${isChallengeSuccess === null ? 'border-slate-300 dark:border-slate-600 focus:border-indigo-500 dark:focus:border-indigo-500 focus:ring-indigo-200 dark:focus:ring-indigo-900/50' : ''}
                  `}
                />
                <button 
                  onClick={checkChallenge}
        ...
        <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 text-slate-400 dark:text-slate-500 text-sm font-medium flex items-center gap-2">
          🖱️ Click hoặc Space để lật
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-16 px-4">
        <button 
          onClick={handlePrev}
          disabled={currentIndex === 0}
          className="flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-all"
        >
          <ArrowLeft size={20} /> Trở lại
        </button>
        <button 
          onClick={handleNext}
          disabled={currentIndex === cards.length - 1}
          className="flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-white bg-slate-800 dark:bg-indigo-600 hover:bg-slate-900 dark:hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-all"
        >
          Tiếp theo <ArrowRight size={20} />
        </button>
      </div>
